<template>
  <transition :name="transition ? 'van-slide-bottom' : ''">
    ...
    >
      <div class="cc-number-keyboard__title van-hairline--top" v-if="title">
        <span>{{ title }}</span>
      </div>
      <i
        ...
        class="cc-hairline"
        :class="{
                    'cc-number-keyboard--active': index === active,
                    'cc-number-keyboard__delete': index === 11 && showDeleteKey
                }"
      />
    </div>
  </transition>
...
<style lang="sass">
  @import "../../style/number-keyboard.scss";
</style>
